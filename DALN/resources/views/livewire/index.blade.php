<div>
    ...
    <main class="lg:min-h-screen lg:mt-0 mt-32 flex items-center lg:mb-0 mb-10">
        <div class="container px-8 mx-auto grid lg:grid-cols-2 gap-10 items-center">
            <div class="space-y-4 lg:order-1 order-2">
                <h1 class="sm:text-6xl text-gray-800 text-4xl font-['poppins'] font-black">{{ __('home.hero_title') }}</h1>
                        ...
                        @if (!auth()->user()->hasRole('user'))
                            <div x-data="{ open: false }" class="flex sm:flex-row flex-col sm:space-x-2 sm:space-y-0 space-y-2 sm:items-end">
                                <div class="form-control">
                                    <label class="label" for="check_in">{{ __('home.check_in') }}</label>
                                    <input min="2022-01-01" class="w-full input" id="check_in" type="date">
                                </div>
                                <div class="form-control">
                                    <label class="label" for="check_out">{{ __('home.check_out') }}</label>
                                    <input min="2022-01-01" class="w-full input" id="check_out" type="date">
                                </div>
                                <button x-on:click="open = true" class="btn">{{ __('home.order') }}</button>
                                    ...
                                    <div x-show="open" x-transition.duration.300ms x-on:click="open = false" class="relative min-h-screen flex items-center justify-center p-4">
                                        <form method="POST" action="#" wire:submit.prevent='reservation' x-on:click.stop x-trap.noscroll.inert="open" class="relative max-w-xl w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4">
                                            <div class="text-center space-y-4">
                                                <i class='bx bx-info-circle text-8xl text-blue-600'></i>
                                                <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">{{ __('home.choose_favorite_room') }}</h2>
                                                <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                                                    {{ __('home.see_room_list') }} <a class="underline" href="{{ route('rooms.index') }}">{{ __('common.here') }}</a>.
                                                </p>
                                            </div>
                                            <div class="grid sm:grid-cols-2 gap-4 items-start">
                                                <div class="form-control">
                                                    <label for="check_in_modal" class="label">{{ __('home.check_in') }}</label>
                                                ...
                                                <div class="form-control">
                                                    <label for="check_out_modal" class="label">{{ __('home.check_out') }}</label>
                                                ...
                                                <div class="form-control">
                                                    <label for="selected_room" class="label">{{ __('home.room') }}</label>
                                                    <select class="select" name="selected_room" id="selected_room" wire:change='setPrice' wire:model='selected_room'>
                                                        @if (count($rooms->where("available", ">", 0)) > 0)    
                                                            <optgroup label="{{ __('room.available') }}">
                                                                @foreach ($rooms->where("available", ">", 0) as $room)
                                                                    <option value="{{ $room->code }}">{{ trans_content($room, 'name') }}</option>  
                                                                @endforeach
                                                            </optgroup>
                                                        @endif
                                                        @if (count($rooms->where("available", "=", 0)) > 0)
                                                            <optgroup label="{{ __('room.unavailable') }}" disabled>
                                                                @foreach ($rooms->where("available", "=", 0) as $room)
                                                                    <option>{{ trans_content($room, 'name') }}</option>  
                                                                @endforeach
                                                            </optgroup>
                                                        @endif
                                                    </select>
                                            ...
                                            @if ($check_in && $check_out && $total_rooms && $selected_room)
                                                <p class="tracking-wide text-gray-600 sm:text-base text-sm">{{ __('room.total_price_stay', ['days' => $totalDays, 'count' => $total_rooms, 'room' => $room_name, 'price' => '$' . $totalPrice]) }}</p>
                                            @endif
                                            <div class="flex space-x-2 justify-center">
                                                <button class="btn">
                                                    {{ __('common.confirm') }}
                        ...
                        <div x-data="{ open: false }" class="flex sm:flex-row flex-col sm:space-x-2 sm:space-y-0 space-y-2 sm:items-end">
                            ...
                            <div x-show="open" style="display: none" x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']" :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
                                ...
                                <div x-show="open" x-transition.duration.300ms x-on:click="open = false" class="relative min-h-screen flex items-center justify-center p-4">
                                    <div x-on:click.stop x-trap.noscroll.inert="open" class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4 text-center">
                                        <i class='bx bx-info-circle text-8xl text-blue-600'></i>
                                        <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">{{ __('auth.email_verification_first') }}</h2>
                                        <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                                            {{ __('auth.must_verify_email') }}
                                        </p>
                                        <div class="flex space-x-2 justify-center">
                    ...
                    <div x-data="{ open: false }" class="flex sm:flex-row flex-col sm:space-x-2 sm:space-y-0 space-y-2 sm:items-end">
                        ...
                        <div x-show="open" style="display: none" x-on:keydown.escape.prevent.stop="open = false" role="dialog" aria-modal="true" x-id="['modal-title']" :aria-labelledby="$id('modal-title')" class="fixed inset-0 overflow-y-auto z-50">
                            ...
                            <div x-show="open" x-transition.duration.300ms x-on:click="open = false" class="relative min-h-screen flex items-center justify-center p-4">
                                <div x-on:click.stop x-trap.noscroll.inert="open" class="relative max-w-md w-full bg-white rounded-xl p-10 overflow-y-auto space-y-4 text-center">
                                    <i class='bx bx-info-circle text-8xl text-blue-600'></i>
                                    <h2 class="text-3xl font-bold text-gray-800" :id="$id('modal-title')">{{ __('auth.must_login_first') }}</h2>
                                    <p class="tracking-wide text-gray-600 sm:text-base text-sm">
                                        {{ __('auth.must_login_before_reservation') }}
                                    </p>
                                    <div class="flex space-x-2 justify-center">
    ...
    <section class="my-20">
        <div class="container px-8 mx-auto space-y-10">
            <div class="space-y-2">
                <h1 class="sm:text-5xl text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full">{{ __('home.our_best_facilities') }}</h1>
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">{{ __('home.facilities_desc') }}</p>
            </div>
                ...
                @foreach ($facilities as $facility)
                    <div class="space-y-4">
                        <div class="aspect-[5/4] rounded-tr-2xl rounded-bl-2xl overflow-hidden">
                            <img class="w-full h-full object-cover hover:scale-110 transition-all duration-300" src="{{ asset("storage/$facility->image") }}" alt="{{ trans_content($facility, 'name') }}">
                        </div>
                        <div class="flex justify-between items-center">
                            <h3 class="font-bold font-['poppins'] text-lg text-gray-800">{{ trans_content($facility, 'name') }}</h3>
                            <a href="{{ route('facilities.index', $facility->code) }}" class="flex items-center gap-1 group">
                                <span class="text-sm text-gray-600 group-hover:underline">{{ __('room.learn_more') }}</span>
    ...
    <section class="lg:my-40 mt-10 mb-20">
        <div class="container px-8 mx-auto lg:space-y-20 space-y-10">
            <div class="space-y-2">
                <h1 class="sm:text-5xl text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full">{{ __('home.favorite_room') }}</h1>
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">{{ __('home.favorite_room_desc') }} <a href="{{ route('rooms.index') }}" class="underline">{{ __('home.click_here_all_options') }}</a></p>.
            </div>
                ...
                @foreach ($favouriteRooms as $room)
                    <div class="flex md:flex-row flex-col lg:gap-20 md:gap-10 gap-6 md:items-center h-full">
                        ...
                        <div class="aspect-[8/6] lg:min-w-[24rem] md:min-w-[20rem] lg:max-w-[24rem] md:max-w-[20rem] w-full rounded-tr-2xl rounded-bl-2xl overflow-hidden">
                            <img class="w-full h-full object-cover hover:scale-110 transition-all duration-300" src="{{ asset("storage/$room->image") }}" alt="{{ trans_content($room, 'name') }}">
                        </div>
                        <div class="flex flex-col justify-between md:h-full gap-1">
                            <h3 class="font-bold text-gray-600">{{ trans_content($room, 'name') }}</h3>
                            <p class="lg:text-6xl text-4xl text-gray-800">{{ trans_content($room, 'description') }}</p>
                            <span class="flex justify-between items-center">
                                <span class="text-sm text-gray-600">{{ (int) $room->total_rooms -  (int) $room->reservations->count()}} {{ __('room.rooms_available') }}</span>
    ...
    <section class="lg:my-40 mt-10 mb-20">
        <div class="container px-8 mx-auto space-y-6">
            <div class="space-y-2">
                <h1 class="sm:text-5xl text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full">{{ __('home.gallery') }}</h1>
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">{{ __('home.gallery_desc') }}</p>
            </div>
    ...
    <section class="lg:my-40 mt-10 mb-20">
        <div class="container px-8 mx-auto space-y-6 text-center">
            <div class="space-y-2">
                <h1 class="sm:text-5xl text-gray-800 text-3xl font-['poppins'] font-black capitalize after:content-[''] after:block after:w-10 after:h-1 after:bg-gray-800 after:rounded-full max-w-max mx-auto">{{ __('home.fascinated') }}</h1>
                <p class="tracking-wide text-gray-600 sm:text-base text-sm">{{ __('home.fascinated_desc') }}</p>
            </div>
            <a href="{{ route('rooms.index') }}" class="btn">{{ __('home.view_all_rooms') }}</a>
        </div>
    </section>
</div>
